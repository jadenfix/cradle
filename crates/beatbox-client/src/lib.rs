...
mod tests {
    ...
    #[tokio::test]
    async fn empty_token_does_not_suppress_legacy_api_key()
    -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let listener = TcpListener::bind("127.0.0.1:0")?;
        let addr = listener.local_addr()?;
        let (request_tx, request_rx) = mpsc::channel();
        ...
        let client = Client::new(format!("http://{addr}/api/"))
            .with_token("")
            .with_api_key("secret");
        let _capabilities = client.capabilities().await?;
        ...
        match server.join() {
            Ok(result) => result?,
            Err(_) => return Err("empty token fallback test server panicked".into()),
        }
        let request = request_rx.recv_timeout(Duration::from_secs(1))?;
